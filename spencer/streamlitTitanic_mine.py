...
# ---------------- Page config ----------------
st.set_page_config(page_title="Titanic Explorer (No-MLs)",
                   page_icon="🚢", layout="wide")
st.title("🚢 Titanic Explorer (No-ML)")
st.caption(
    "Filter → Summarise → Visualise. Built with Streamlit + pandas + matplotlib (no ML).")
...
def load_titanic() -> pd.DataFrame:
    ...
    cat_cols = ["sex", "class", "embark_town", "who",
                "adult_male", "alone", "deck", "embarked", "alive"]
    for c in df.columns:
        if c in cat_cols:
            df[c] = df[c].astype("category")
    # Rename a couple for clarity in UI
    df = df.rename(columns={"embark_town": "embark_town",
                            # pclass isn't in seaborn titanic (they use 'class')
                            "pclass": "pclass"})
    return df
...
with st.sidebar:
    ...
    sex = st.multiselect("Sex", options=sorted(
        df["sex"].dropna().unique().tolist()), default=None)
    cls = st.multiselect("Class", options=sorted(
        df["class"].dropna().unique().tolist()), default=None)
    embarked = st.multiselect("Embarked (town)", options=sorted(
        df["embark_town"].dropna().unique().tolist()), default=None)

    # Numeric ranges
    age_min, age_max = float(df["age"].min(skipna=True)), float(
        df["age"].max(skipna=True))
    fare_min, fare_max = float(df["fare"].min(
        skipna=True)), float(df["fare"].max(skipna=True))
    age_range = st.slider("Age range", min_value=0.0, max_value=float(np.ceil(age_max)), value=(float(np.floor(
        age_min)) if not np.isnan(age_min) else 0.0, float(np.ceil(age_max)) if not np.isnan(age_max) else 80.0))
    fare_range = st.slider("Fare range", min_value=0.0, max_value=float(
        np.ceil(fare_max)), value=(0.0, float(np.ceil(fare_max))))
...
if "age" in fdf.columns:
    fdf = fdf[(fdf["age"].isna()) | (
        (fdf["age"] >= age_range[0]) & (fdf["age"] <= age_range[1]))]
if "fare" in fdf.columns:
    fdf = fdf[(fdf["fare"].isna()) | (
        (fdf["fare"] >= fare_range[0]) & (fdf["fare"] <= fare_range[1]))]
...
# ---------------- Tabs ----------------
tab_overview, tab_insights, tab_charts, tab_answers = st.tabs(
    ["Overview", "Survival Insights", "Charts", "Answers"])
...
with tab_overview:
    ...
    st.dataframe(fdf.head(page_size), use_container_width=True,
                 hide_index=not show_row_index)
...
with tab_insights:
    ...
    with left:
        ...
        cat_cols = [c for c in fdf.select_dtypes(
            include=["category", "object"]).columns if c not in ["alive"]]
        group_col = st.selectbox("Group by (categorical)", options=cat_cols if cat_cols else [
                                 "sex"], index=(cat_cols.index("sex") if "sex" in cat_cols else 0))
        # Aggregated survival rate by group
        if "survived" in fdf.columns:
            grp = fdf.groupby(group_col, dropna=False)["survived"].mean().mul(100).reset_index().rename(
                columns={"survived": "survival_rate_%"}).sort_values("survival_rate_%", ascending=False)
    ...
    with right:
        ...
        if "survived" in fdf.columns:
            comp = fdf.groupby("survived")[["age", "fare"]].mean(
                numeric_only=True).round(2)
            st.dataframe(comp, use_container_width=True)
        st.markdown("**Download filtered data**")
        st.download_button("⬇️ CSV", data=fdf.to_csv(index=False).encode(
            "utf-8"), file_name="titanic_filtered.csv", mime="text/csv")

    st.divider()
    st.subheader("Crosstab: Survived vs chosen group")
    if "survived" in fdf.columns:
        # Use the same group_col
        ct = pd.crosstab(fdf[group_col], fdf["survived"],
                         dropna=False, normalize="index").mul(100).round(1)
        ...
        for surv_value in counts.columns:
            ...
            ax2.bar(counts.index.astype(str), vals,
                    bottom=bottom, label=f"survived={surv_value}")
...
with tab_charts:
    ...
    chart_type = st.selectbox("Chart type", [
                              "Bar (survival rate by group)", "Scatter (Age vs Fare)", "Line (sorted Fare or Age)"])
    ...
    if chart_type == "Bar (survival rate by group)":
        ...
        if cat_cols and "survived" in fdf.columns:
            group_col2 = st.selectbox("Group by", options=cat_cols, index=(
                cat_cols.index("class") if "class" in cat_cols else 0), key="bar_group")
            grp2 = fdf.groupby(group_col2, dropna=False)["survived"].mean().mul(
                100).reset_index().rename(columns={"survived": "survival_rate_%"}).dropna()
        ...
        sdf = fdf[["age", "fare", "survived", "sex"]].dropna(
            subset=["age", "fare"]).copy()
        fig4, ax4 = plt.subplots(figsize=(7, 4))
        if color_by == "(none)":
            ax4.scatter(sdf["age"], sdf["fare"], alpha=0.8)
        else:
            for level, sub in sdf.groupby(color_by, dropna=False):
                ax4.scatter(sub["age"], sub["fare"],
                            alpha=0.8, label=str(level))
        ...
        s = fdf[series].dropna().sort_values(ascending=(
            order_by == "increasing")).reset_index(drop=True)
...
with tab_answers:
    ...
    st.markdown("#### Survival Rate by:")

    female = df[(fdf['sex'] == 'female')]
    surv_f_rate = female["survived"].mean() * 100
    st.markdown(f"Sex: {surv_f_rate}")
...
st.session_state.notes = st.text_area(
    "Write observations you’d like to remember", value=st.session_state.notes, height=120)
